...
#define CHANNEL 1
//#define key_on_threshold 500
//#define KEY_OFF_THRESHOLD 200
...
int sensorPins[NUM_SENSORS] = {15, 16, 17, 18, 19, 22, 23, 33, 32, 25};
...
unsigned long timeSinceFirstButtonPress=0; // the time we need to wait
int numButtonsPressed = 0;


int maxDutyCycle = 1000;
int minDutyCycle = 300;

bool keyIsOn[7] = {false, false, false, false, false, false, false};
int lastPressureReading[7] = {0, 0, 0, 0, 0, 0 , 0};



int lastPitchWheelMessage = 0;

int tares[NUM_SENSORS] = {};
...
void loop()                    
{
    ...
    for (int i=0; i<3; i++) 
    {
        ...
        {
            case 0:
                key_on_threshold = potVal - 260; // 260 is what pot is resting at cuz it's broke
                break;
            case 1:
        ...
        if (bounceButtons[i].update()) 
        {
            ...
            if (bounceButtons[i].risingEdge() && buttonStates[i] == HIGH) 
            {
                ...
                for (int i=0; i< NUM_SENSORS; i++)
                {
                    if (keyIsOn[i])
                    {
                        int lastMidiNote = lastBaseNote + scale[i];
                        int newMidiNote = currentBaseNote + scale[i];
                        usbMIDI.sendNoteOff(lastMidiNote, 0, i+1); // turn off all notes when going to next scale
                        usbMIDI.sendNoteOn(newMidiNote, lastPressureReading[i], i+1); // turn on the note in the new scale
                    }
